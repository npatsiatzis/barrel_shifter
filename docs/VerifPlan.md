...
1. **Scope**
   ...
   This document establishes the verification plan for the barrel shifter design specified in
...
1. **Non-government Documents**
   ...
   Document #: barrel_shifter_req_001(Specification.md), Requirement Specification for barrel shifter.
...
to mimic a barrel shifter in a system environment. The environment will perform the
following transactions at pseudo-random intervals: 
   1. Create requests for SLL operations 
   2. Create requests for SLA operations
   3. Create requests for SRL operations
   4. Create requests for SRA operations
   3. Force resets
   ...
   | :------: | :------: | :------: | :------: |
   | Reset | Directed Test | set reset signal active (high) | All components of design enter reset state |
   | Shift Operations | Constrained Random |Coverage Driven (full cross-coverage for all(4) operations with all shift amounts for all input data) | *assert* observed data = expected data, for each test |
